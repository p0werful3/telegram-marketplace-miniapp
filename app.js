...
async function registerUser() {
    try {
        ...
        if (!response.ok) {
            tg.showAlert("Помилка реєстрації");
            return;
        }

        document.getElementById("profile-status").textContent = "Зареєстровано";
        tg.showAlert("Користувача зареєстровано");
        fillProfile();
    } catch (error) {
        console.error(error);
        tg.showAlert("Не вдалося підключитися до API");
    }
}
...
async function createProduct() {
    ...
    if (!title || !description || !price || !category) {
        tg.showAlert("Заповни всі обов'язкові поля");
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            tg.showAlert(data.detail || "Не вдалося створити оголошення");
            return;
        }

        tg.showAlert("Оголошення створено");
        ...
        console.error(error);
        tg.showAlert("Помилка підключення до API");
    }
}
...
async function addToCart(productId) {
    try {
        ...
        if (!response.ok) {
            tg.showAlert(data.detail || "Не вдалося додати товар");
            return;
        }

        tg.showAlert("Товар додано до кошика");
    } catch (error) {
        console.error(error);
        tg.showAlert("Помилка підключення до API");
    }
...
async function buyProduct(productId) {
    try {
        ...
        if (!response.ok) {
            tg.showAlert(data.detail || "Помилка покупки");
            return;
        }
        ...
        console.error(error);
        tg.showAlert("Не вдалося виконати покупку");
    }
}
