...
function sellerBadgeLabel(soldProducts = 0, ratingCount = 0) {
    if (Number(soldProducts) >= 10) return "Топ продавець";
    if (Number(soldProducts) >= 3 && Number(ratingCount) >= 3) return "Надійний продавець";
    return "Новий продавець";
}

function getTelegramInitData() {
    const fromTg = tg?.initData || window.Telegram?.WebApp?.initData || "";
...
function extractTelegramUser() {
    const directUser = tg?.initDataUnsafe?.user || window.Telegram?.WebApp?.initDataUnsafe?.user || null;
    ...
    try {
        ...
        console.error("Telegram user parse error:", error);
        return null;
    }
}

function updateTelegramLoginButton() {
    const tgButton = $("tg-login-btn");
    const tgHint = $("tg-login-hint");
    if (!tgButton) return;

    const hasTelegramContext = Boolean(getTelegramInitData() || extractTelegramUser());
    tgButton.disabled = false;
    tgButton.textContent = hasTelegramContext ? "Увійти через Telegram" : "Увійти через Telegram";
    if (tgHint) {
        tgHint.textContent = hasTelegramContext
            ? "Швидкий вхід через ваш Telegram акаунт"
            : "Працює найкраще всередині Telegram Mini App";
    }
}
...
function initTelegramWebApp() {
    try {
        ...
        telegramUser = extractTelegramUser();
        updateTelegramLoginButton();
...
async function loginWithTelegram() {
    ...
    if (!tgUser && !initData) {
        showAlert("Відкрийте застосунок через кнопку бота в Telegram, щоб увійти цим способом");
        return;
    }
    ...
    try {
        ...
        const data = await safeFetch(`${API_BASE}/auth/telegram`, {
            ...
            body: JSON.stringify({
                ...
                full_name: tgUser ? `${tgUser.first_name || ""} ${tgUser.last_name || ""}`.trim() : null,
...
async function loadMyReviews() {
    ...
    list.innerHTML = `<div class="empty-card">Завантаження...</div>`;
    try {
        const data = await safeFetch(`${API_BASE}/users/${currentUser.id}/reviews`);
        currentUser.rating_sum = (data.rating_count || 0) * (data.rating || 0);
        currentUser.rating_count = data.rating_count || 0;
        fillProfile();
        if (!data.items?.length) {
            list.innerHTML = `<div class="empty-card">Поки що відгуків немає</div>`;
            return;
        }
        list.innerHTML = data.items.map(item => `
            <div class="card">
                <div class="card-body">
                    <div class="review-top-row"><strong>${escapeHtml(item.buyer_full_name || item.buyer_username || "Покупець")}</strong><span class="review-rating">⭐ ${escapeHtml(String(item.rating))}</span></div>
                    ${item.buyer_username ? `<div class="card-seller">@${escapeHtml(item.buyer_username)}</div>` : ""}
                    ${item.comment ? `<div class="card-description no-clamp">${escapeHtml(item.comment)}</div>` : `<div class="card-seller">Без коментаря</div>`}
                    <div class="card-seller">Дата: ${formatDate(item.created_at)}</div>
                </div>
            </div>
        `).join("");
    } catch (error) {
...
function toggleMyReviews() {
    ...
    if (!wrap || !btn) return;
    const shouldOpen = wrap.classList.contains("hidden");
    wrap.classList.toggle("hidden", !shouldOpen);
    btn.textContent = shouldOpen ? "Сховати мої відгуки" : "Мої відгуки";
    if (shouldOpen) loadMyReviews();
}
...
async function loadCart() {
    ...
    try {
        ...
        cartList.innerHTML = data.items.map(item => `
            <div class="card cart-mini-card">
                <div class="cart-mini-layout">
                    ${isValidUrl(item.image_url) ? `<img class="cart-mini-image" src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.title)}">` : `<div class="cart-mini-image cart-mini-placeholder">Без фото</div>`}
                    <div class="cart-mini-main">
                        <div class="cart-mini-top"><h3 class="card-title cart-mini-title">${escapeHtml(item.title)}</h3><p class="card-price cart-mini-price">${formatPrice(item.price, item.currency)}</p></div>
                        ${item.seller_username ? `<p class="card-seller">Продавець: @${escapeHtml(item.seller_username)}</p>` : ""}
                        <div class="card-actions cart-mini-actions">
                            <button class="buy-btn" onclick="buyProduct(${Number(item.product_id)})">Купити</button>
                            ${item.seller_username ? `<a class="contact-btn contact-link" href="https://t.me/${escapeHtml(item.seller_username)}" target="_blank" rel="noopener noreferrer">Написати</a>` : ""}
...
async function buyAllFromCart() {
    ...
    try {
        const data = await safeFetch(`${API_BASE}/cart/${currentUser.id}`);
        if (!data?.items?.length) {
            showAlert("Кошик порожній");
            return;
        }
        for (const item of data.items) {
            await buyProduct(Number(item.product_id));
        }
        await loadCart();
    } catch (error) {
